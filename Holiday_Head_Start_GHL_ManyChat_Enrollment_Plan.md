...
2. **Not-yet-a-member path:** respond warmly rather than treating the person as ineligible. Explain that the full Holiday Head Start member experience is reserved for paid MacroFitters, invite them to begin with a personalized nutrition plan, and route them to the focused prospect conversion page rather than direct checkout: **Show Me the Holiday Plan** → `https://macrofitprogram.com/holiday-head-start/join?utm_source=manychat&utm_medium=instagram-dm&utm_campaign=holiday-head-start-2026&utm_content=non-member-learn`. A high-intent second button can deep-link to `#membership` on that page. Do not add this branch to the paid-member challenge workflow.
...
6. On the not-yet-member branch, remove old first-name, phone, email, discount-code, and direct-checkout cards. Replace them with a concise MacroFit sales bridge plus two tracked buttons: **Show Me the Holiday Plan** and **See Membership Options**, both pointing to the dedicated prospect conversion page.
...
| ManyChat non-member branch | The full revised flow copy is documented in `Holiday_Head_Start_ManyChat_NonMember_Flow_Copy.md`. It removes the legacy contact-capture/code sequence and routes prospects to the focused MacroFit conversion page. | In ManyChat, replace the legacy No branch with the sales bridge, the ManyChat-only interest tag, and the two tracked buttons. Test Instagram and Facebook Messenger on mobile. |
